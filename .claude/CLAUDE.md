...
**Graph drills 4/5 done.** `greedy_nn_walk` SOLVED — the M3 pivot drill (mini-HNSW single layer). Greedy hill-descent toward query, squared-Euclid, oracle = `brute_nn` (linear scan, O(N) ground truth). First attempt was best-first/heap (wrong algo) — kept + renamed `bestfit_nn_walk`; real greedy = single pointer, include current in candidates, step to `min`, stop at local min (beam width 1, NO heap). `min` over `(dist,id)` tuples → id tiebreak prevents oscillation. **Recall lesson captured in code:** trap graph `[[1],[0,2],[1,4],[2,4],[1]]` makes node 3 (true NN) unreachable → greedy stuck at 4 (`greedy=4 != brute=3`), asserted the *disagreement* in a separate block from the agreement loop. That gap = recall@k → why HNSW stacks layers + widens beam (`ef`, drill 5). Next: drill 5 `beam_search` (= HNSW efSearch, two heaps).

## Current state (2026-06-19) — drill 3
**Graph drills 3/5 done.** `dijkstra` SOLVED — `dist[i]`=min weight src→i (`-1` unreachable), Dijkstra == Bellman-Ford oracle. Core realisation: **Dijkstra = BFS with 3 swaps** — `deque`→min-heap, `popleft`→`heappop` (globally-closest not oldest), `+1`→`+weight` pushing cumulative `(dist_so_far, node)`; ONE global heap for whole search. Big bug (kept as dead-code ref `realaxed_dfs`): first attempt built a *fresh heap per recursive call* → no global frontier → accidentally relaxed-DFS not Dijkstra. Oracle bug: Bellman-Ford used `inf` internally, `int(inf)` → OverflowError on unreachable — fix = `[-1 if v==inf else int(v) ...]` (convert sentinel at boundary). Banked: *relax edge* = tighten `dist[v]` if path-via-u cheaper; *Dijkstra finalises on pop → only valid for weights ≥0; BF relaxes all edges n−1× → handles negatives + detects neg-cycle*. Lazy-skip nuance: cleaner to `if d>distance[u]: continue`. Next: drill 4 `greedy_nn_walk` (mini-HNSW single layer).

## Current state (2026-06-19) — drill 2
